...
class Medoo
{
    ...
    public function insert($data) {
        ...
        $this->exec('INSERT INTO `' . $this->getTrueTableName() . '` (`' . $keys . '`) VALUES (' . implode($values, ', ') . ')');
    ...
    public function update($data, $where = NULL) {
        ...
        return $this->exec('UPDATE `' . $this->getTrueTableName() . '` SET ' . implode(', ', $fields) . $this->where_clause($where));
    }

    public function delete($where) {
        return $this->exec('DELETE FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where));
    }
    ...
    public function replace($columns, $search = NULL, $replace = NULL, $where = NULL) {
        ...
        return $this->exec('UPDATE `' . $this->getTrueTableName() . '` SET ' . $replace_query . $this->where_clause($where));
    }
    ...
    public function has($where) {
        return $this->query('SELECT EXISTS(SELECT 1 FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where) . ')')->fetchColumn() === '1';
    }

    public function count($where = NULL) {
        return 0 + ($this->query('SELECT COUNT(*) FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where))->fetchColumn());
    }

    public function max($column, $where = NULL) {
        return 0 + ($this->query('SELECT MAX(`' . $column . '`) FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where))->fetchColumn());
    }

    public function min($column, $where = NULL) {
        return 0 + ($this->query('SELECT MIN(`' . $column . '`) FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where))->fetchColumn());
    }

    public function avg($column, $where = NULL) {
        return 0 + ($this->query('SELECT AVG(`' . $column . '`) FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where))->fetchColumn());
    }

    public function sum($column, $where = NULL) {
        return 0 + ($this->query('SELECT SUM(`' . $column . '`) FROM `' . $this->getTrueTableName() . '`' . $this->where_clause($where))->fetchColumn());
    }
